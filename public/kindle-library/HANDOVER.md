...
- モバイル対応（v0.7.0）: FS API非対応ブラウザ（iOS Safari等）では「＋ PDFを追加」
  ...
  640px以下のレスポンシブCSSあり
- ズーム中の誤操作防止（v0.7.1）: visualViewport.scaleでピンチズームを検出し、
  ズーム中（アプリズーム>100%含む）はタップ/スワイプのページ遷移を無効化。
  ページ移動は左右端ダブルタップ、中央ダブルタップでアプリズームリセット、
  単タップ時はヒントトースト（5秒に1回）。pdf-reader側にも同実装（v0.5.3）
