...
export const RELEASE_NOTES: { version: string; date: string; changes: string[] }[] = [
  ...
  },
  {
    ...
    date: '2026-08-13',
    changes: [
      '検索タブに「タグ一覧」を追加 — 全タグを冊数付きで表示(冊数順/名前順)、タップでそのタグの本を検索',
    ],
  },
  {
    version: 'v1.15.1',
  ...
  },
  {
    ...
    date: '2026-08-13',
    changes: [
      '検索の表示切替ボタンに文字ラベルを追加(リスト/書影/パラパラ)',
      'パラパラビュー内に操作ヒントを表示、使い方ガイドにも追記',
    ],
  },
  {
    version: 'v1.14.3',
...
・リスト … 通常の一覧
・書影 … 書影をグリッド表示(タップで詳細)
・パラパラ … 全画面で書影を1冊ずつめくるモード。
  スワイプまたは←→キーでめくり、▶ボタンで自動再生(パラパラ漫画のように流れます)。
  書影をタップすると詳細、✕やEscで閉じます。
  検索の絞り込み・ソートがそのまま反映されるので
  「★4以上を評価順にパラパラ」のような眺め方ができます。
